...
broadband_county$avail <- as.double(broadband_county$broadband_availability_per_fcc)

broadband_county_sum <- broadband_county %>% 
  group_by(state) %>%
  summarise(mean(avail))
...
ggplot() + geom_sf(data = states, color="black",
